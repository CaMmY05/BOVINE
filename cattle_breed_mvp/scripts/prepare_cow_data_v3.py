"""
Prepare V3 cow data with 5 breeds for training
"""
...
def prepare_cow_data_v3():
    ...
    for breed_dir in breed_dirs:
        ...
        for split_dir in breed_dir.iterdir():
            if split_dir.is_dir():
                for img_file in split_dir.glob("*.jpg"):
                    all_images.append(img_file)
                    all_labels.append(breed_name)
                for img_file in split_dir.glob("*.jpeg"):
                    all_images.append(img_file)
                    all_labels.append(breed_name)
    ...
    print(f"📊 Found {len(all_images)} total images across {len(breed_dirs)} breeds")
    ...
    for breed in unique_breeds:
        train_count = len([x for x in X_train if x.parent.name == breed])
        val_count = len([x for x in X_val if x.parent.name == breed])
        test_count = len([x for x in X_test if x.parent.name == breed])
        total = train_count + val_count + test_count
        percentage = (total / len(X_train + X_val + X_test)) * 100
        
        print(f"{breed:<20} {train_count:<8} {val_count:<8} {test_count:<8} {total:<8} {percentage:<6.1f}%")
    
    print("-" * 70)
    print(f"{'TOTAL':<20} {len(X_train):<8} {len(X_val):<8} {len(X_test):<8} {len(X_train + X_val + X_test):<8} {'100.0':<6}")
    ...
    print(f"\n🎯 Ready for training!")
    print(f"📊 Total images: {len(X_train + X_val + X_test)}")
    ...
    return {
        ...
        "total_size": len(X_train + X_val + X_test),
