"""
Prepare V3 cow data with 5 breeds for training - FIXED VERSION
"""
...
def prepare_cow_data_v3():
    ...
    # Process each breed - the breeds are the subdirectories
    breed_dirs = [d for d in source_dir.iterdir() if d.is_dir()]
    
    print(f"📁 Found breed directories: {[d.name for d in breed_dirs]}")
    
    for breed_dir in breed_dirs:
        breed_name = breed_dir.name
        
        # Skip if it's a split directory
        if breed_name in ['train', 'valid', 'test']:
            continue
        ...
        for split_dir in breed_dir.iterdir():
            if split_dir.is_dir():
                print(f"  📂 {split_dir.name}: {len(list(split_dir.glob('*.jpg')))} images")
