...
st.markdown("""
This MVP system uses YOLO for cattle detection and deep learning for breed classification.
...
model_arch = st.sidebar.radio(
    ...
    ["EfficientNet-B0 V1", "EfficientNet-B0 V2", "EfficientNet-B0 V3", "ResNet18", "ResNet32"],
    index=1
)

# Map model architecture to supported animal types
MODEL_ANIMAL_SUPPORT = {
    "EfficientNet-B0 V1": ["🐄 Cow"],  # Only supports Cows
    "EfficientNet-B0 V2": ["🐄 Cow", "🐃 Buffalo"],
    "EfficientNet-B0 V3": ["🐄 Cow"],  # Only supports Cows
    "ResNet18": ["🐄 Cow", "🐃 Buffalo"],
    "ResNet32": ["🐄 Cow", "🐃 Buffalo"]
}
...
# Animal type selector (only show supported options)
...
MODEL_PATHS = {
    # EfficientNet Models
    "EfficientNet-B0 V1": {
        "Cow": "cow_classifier_v2"  # Using V2 as V1 since it's the original model
    },
    "EfficientNet-B0 V2": {
    ...
    "ResNet32": {
        "Cow": "resnet34_cow_v1",  # Using resnet34 as resnet32 for now
...
ARCH_TO_MODEL = {
    "EfficientNet-B0 V1": "efficientnet_b0",
    "EfficientNet-B0 V2": "efficientnet_b0",
    "EfficientNet-B0 V3": "efficientnet_b0",
    "ResNet18": "resnet18",
    "ResNet32": "resnet32"
}

# Get the model directory
model_dir = MODEL_PATHS[model_arch][animal_type.replace("🐄 ", "").replace("🐃 ", "")]
model_path = f"models/classification/{model_dir}"
classes_path = f"{model_path}/classes.json"

# For ResNet models, we need to look in checkpoints directory
if model_arch in ["ResNet18", "ResNet32"]:
    if os.path.exists(f"{model_path}/checkpoints/best_model.pth"):
        model_path = f"{model_path}/checkpoints/best_model.pth"
    elif os.path.exists(f"{model_path}/best_model.pth"):
        model_path = f"{model_path}/best_model.pth"
    else:
        model_path = f"{model_path}/final_model.pth"
else:
...
model_version = f"{model_arch} ({animal_type.replace('🐄 ', '').replace('🐃 ', '')})"
...
if not models_exist:
    st.warning(f"⚠️ Model not found at {model_path}. Please ensure the model files exist.")
    st.info("""
    **Available Models:**
    - EfficientNet-B0 V1: breed_classifier_v1/
    - EfficientNet-B0 V2: cow_classifier_v2/
    - EfficientNet-B0 V3: cow_classifier_v3/
    - ResNet18: resnet18_*_v1/
    - ResNet32: resnet34_*_v1/ (using ResNet34 as ResNet32)
    """)
    st.stop()
...
models_exist = os.path.exists(classification_model_path) and os.path.exists(classes_path)

if not models_exist:
    st.warning("⚠️ Models not found! Please train the model first using `train_classifier.py`")
    st.info("""
    **Setup Steps:**
    1. Prepare your dataset: `python scripts/prepare_data.py`
    2. Extract ROIs: `python scripts/extract_roi.py`
    3. Train classifier: `python scripts/train_classifier.py`
    """)
else:
    # Load model (cached with animal type as key)
    @st.cache_resource
    def load_predictor(animal_type_key, model_path, classes_path_param, model_arch_name):
        from scripts.inference import CattleBreedPredictor
        model_arch = ARCH_TO_MODEL[model_arch_name]
        return CattleBreedPredictor(
            detection_model_path=detection_model_path,
            classification_model_path=model_path,
            classes_path=classes_path_param,
            model_arch=model_arch
        )
    
    try:
        # Pass animal_type and model_arch as keys to ensure separate cache
                ...
                try:
                    ...
                    if not rois:
                        ...
                        for idx, (roi, box_info) in enumerate(zip(rois, boxes_info)):
                            ...
                            # Show three-view analysis if enabled
                            if show_three_views:
        ...
        st.sidebar.info("""
        **Cattle Breed Recognition MVP**
        
        - Detection: YOLOv8
        - Classification: EfficientNet-B0
        - Features: Multi-view analysis
